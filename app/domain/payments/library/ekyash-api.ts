...
const getJWTToken = async (kyash: EKyashEntity) => {
  ...
  const signature = enc.Base64.stringify(HmacSHA256(`${header}.${data}`, ekyash.headers.IMGE));
 ...
 * @returns
 */
const getAuthorization = async (data: EKyashEntity): Promise<AuthorizationResponse> => {
